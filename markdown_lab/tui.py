    ...
    def compose(self) -> ComposeResult:
        with Vertical():
            ...
            with Horizontal(classes="button-row"):
                yield Button(
                    "[START] Start Conversion", variant="primary", id="start_conversion"
                )
                yield Button(
                    "[CANCEL] Cancel", variant="error", id="cancel_conversion", disabled=True
                )
...
class BatchURLTab(Container):
    ...
    def compose(self) -> ComposeResult:
        with Vertical():
            ...
            with Horizontal(classes="button-row"):
                yield Button("[START] Start Batch", variant="primary", id="start_batch")
                yield Button(
                    "[CANCEL] Cancel", variant="error", id="cancel_batch", disabled=True
                )
...
class SitemapTab(Container):
    ...
    def compose(self) -> ComposeResult:
        with Vertical():
            ...
            with Horizontal(classes="button-row"):
                yield Button("[DISCOVER] Discover URLs", id="discover_urls")
                yield Button("[START] Start Sitemap", variant="primary", id="start_sitemap")
                yield Button(
                    "[CANCEL] Cancel", variant="error", id="cancel_sitemap", disabled=True
                )
...
class LogTab(Container):
    ...
    def compose(self) -> ComposeResult:
        with Vertical():
            ...
            with Horizontal(classes="button-row"):
                yield Button("[CLEAR] Clear Logs", id="clear_logs")
                yield Button("[SAVE] Save Logs", id="save_logs")
...
class ConfigTab(Container):
    ...
    def compose(self) -> ComposeResult:
        with Vertical():
            ...
            with Horizontal(classes="button-row"):
                yield Button("[SAVE] Save Config", variant="primary", id="save_config")
                yield Button("[RESET] Reset to Defaults", id="reset_config")
                yield Button("[SHOW] Show Current", id="show_config")
...
class MarkdownLabTUI(App):
    ...
    def on_mount(self) -> None:
        ...
        self.log_to_app("[START] Markdown Lab TUI started")
    ...
    def conversion_complete(self, success: bool, message: str) -> None:
        ...
        if success:
            ...
            self.notify("[SUCCESS] Conversion completed successfully!", severity="information")
        else:
            self.log_to_app(f"[ERROR] {message}")
            self.notify(f"[ERROR] Conversion failed: {message}", severity="error")
    ...
    def handle_start_conversion(self) -> None:
        ...
        if not url:
            self.notify("[ERROR] Please enter a URL", severity="error")
            return
        ...
        if self.conversion_worker.start_conversion(url, config):
            self.log_to_app(f"[START] Starting conversion of {url}")
            self.query_one("#start_conversion", Button).disabled = True
            self.query_one("#cancel_conversion", Button).disabled = False
        else:
            self.notify("[WARNING] Conversion already in progress", severity="warning")

    def handle_cancel_conversion(self) -> None:
        """Cancel ongoing conversion."""
        self.log_to_app("[CANCELLED] Conversion cancelled by user")
        self.query_one("#start_conversion", Button).disabled = False
        self.query_one("#cancel_conversion", Button).disabled = True
    ...
    def handle_clear_logs(self) -> None:
        ...
        log_widget = self.query_one("#app_log", Log)
        log_widget.clear()
        self.log_to_app("[CLEARED] Logs cleared")

    def handle_save_logs(self) -> None:
        """Save application logs to file."""
        # This would save logs to a file
        self.log_to_app("[SAVED] Logs saved to markdown_lab_tui.log")
        self.notify("[SAVED] Logs saved successfully!", severity="information")

    def handle_save_config(self) -> None:
        """Save current configuration."""
        self.log_to_app("[SAVED] Configuration saved")
        self.notify("[SAVED] Configuration saved!", severity="information")

    def handle_reset_config(self) -> None:
        """Reset configuration to defaults."""
        self.log_to_app("[RESET] Configuration reset to defaults")
        self.notify("[RESET] Configuration reset!", severity="information")

    def handle_show_config(self) -> None:
        """Show current configuration."""
        self.log_to_app("[SHOW] Current configuration displayed")
    ...
    def handle_discover_urls(self) -> None:
        ...
        if not url:
            self.notify("[ERROR] Please enter a base URL", severity="error")
            return

        self.log_to_app(f"[DISCOVER] Discovering URLs from {url}")
        self.notify("[DISCOVER] Discovering URLs...", severity="information")
        ...
        self.log_to_app(f"[FOUND] Found {len(dummy_urls)} URLs")
        self.notify(f"[FOUND] Found {len(dummy_urls)} URLs!", severity="information")

    def handle_start_sitemap(self) -> None:
        """Start sitemap conversion."""
        self.log_to_app("[START] Starting sitemap conversion")
        self.notify(" Sitemap conversion started!", severity="information")

    def handle_start_batch(self) -> None:
        """Start batch conversion."""
        self.log_to_app("[START] Starting batch conversion")
